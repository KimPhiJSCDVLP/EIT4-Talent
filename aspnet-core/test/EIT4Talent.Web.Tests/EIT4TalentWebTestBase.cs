...
using EIT4Talent.Models.TokenAuth;
...
{
    public abstract class EIT4TalentWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static EIT4TalentWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(EIT4TalentWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<EIT4TalentDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<EIT4TalentDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<EIT4TalentDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<EIT4TalentDbContext, Task<T>> func)
        {
            T result;
